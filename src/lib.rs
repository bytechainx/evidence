...
fn receipt_line(seq: u64, record: &EvidenceRecord, binding: Option<&ReceiptBinding>) -> String {
...
mod tests {
    use super::{
        file_identity, sha256_hex, EvidenceRecord, EvidenceStore, FileEvidenceStore,
        FileProcessLock,
    };
    ...
    fn file_store_rejects_existing_cross_process_lock() {
        ...
        assert!(matches!(
            ...
            Err(super::EvidenceError::PathAlreadyOpen)
        ));
        drop(first);
    ...
    fn file_store_rejects_open_from_another_process() {
        ...
        let mut child = Command::new(std::env::current_exe().expect("test binary"))
            .args(["--exact", "tests::cross_process_lock_holder", "--nocapture"])
